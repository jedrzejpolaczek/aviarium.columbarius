"""Time the with_canonical NL join with the FULL column list.

Uses the exact same CTEs as silver_cards.sql up through with_canonical,
then just counts rows. No write, no window function, no format_count.
This isolates the NL join cost with real data volume.
"""
import time, duckdb
from pathlib import Path

BRONZE = "data/bronze/cards.duckdb"
FULL_SQL = Path("src/data/cards/storage/silver/sql/silver_cards.sql").read_text()

# Extract just the CTEs up to and including with_canonical, add a COUNT
UP_TO_WITH_CANONICAL = """
WITH
mtgjson_filtered AS (
...
mtgjson AS (
    SELECT
        ...
        list_filter(
            ...
            x -> list_contains(['Legendary','Basic','Snow','World','Elite','Host'], x)
...
scryfall AS (
    SELECT
        ...
        TRIM(lang) AS lang,
        CASE TRIM(lang)
            WHEN 'en' THEN 'English' WHEN 'es' THEN 'Spanish' WHEN 'fr' THEN 'French'
...
joined AS (
    SELECT
        ...
        s.oracle_id, s.type_line, s.illustration_id, s.set_id, s.set_name,
        s.set_type, s.scryfall_uri, s.lang, s.tcgplayer_id, s.cardmarket_id,
        s.penny_rank, s.is_variation, s.is_booster, s.game_changer,
        s.all_parts, s.card_faces, s.multiverse_ids, s.games,
...
SQL_HASH = UP_TO_WITH_CANONICAL + """,
with_canonical AS (
    SELECT j.*, CASE WHEN j.uuid IS NOT NULL THEN j.uuid ELSE cm.uuid END AS canonical_uuid
    FROM joined j
    LEFT JOIN canonical_map cm
           ON j.set_code = cm.set_code AND j.collector_number = cm.collector_number
)
SELECT count(*), count(canonical_uuid) FROM with_canonical
"""
...
print("Full column list — with_canonical ONLY (no write, no window, no format_count):\n")
t_nl   = run("NL join",   SQL_NL)
t_hash = run("Hash join", SQL_HASH)

ratio = t_nl / t_hash if t_hash > 0 else float("inf")
print(f"\n  Speedup: {ratio:.0f}x  ({t_nl:.1f}s -> {t_hash:.1f}s)")
