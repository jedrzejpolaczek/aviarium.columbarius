"""Measure each phase of silver_cards.sql by forcing full column evaluation.
Uses the actual (buggy) $.scryfallId SQL to match real pipeline.
"""
import time, duckdb
...
mtgjson AS (
    SELECT
        ...
        NULLIF(TRIM(power), '_') AS power, NULLIF(TRIM(toughness), '_') AS toughness,
        NULLIF(TRIM(loyalty), '_') AS loyalty, NULLIF(TRIM(defense), '_') AS defense,
        LOWER(TRIM(rarity)) AS rarity, LOWER(TRIM(border_color)) AS border_color,
...
scryfall AS (
    SELECT
        ...
        TRIM(lang) AS lang,
        CASE TRIM(lang) WHEN 'en' THEN 'English' WHEN 'es' THEN 'Spanish'
            WHEN 'fr' THEN 'French' WHEN 'de' THEN 'German' WHEN 'it' THEN 'Italian'
            WHEN 'pt' THEN 'Portuguese' WHEN 'ja' THEN 'Japanese' WHEN 'ko' THEN 'Korean'
            WHEN 'ru' THEN 'Russian' WHEN 'zhs' THEN 'Chinese Simplified'
            WHEN 'zht' THEN 'Chinese Traditional' ELSE TRIM(lang) END AS language,
...
,joined AS (
    SELECT
        ...
        s.oracle_id, s.type_line, s.illustration_id, s.set_id, s.set_name, s.set_type,
        s.scryfall_uri, s.lang, s.tcgplayer_id, s.cardmarket_id, s.penny_rank,
        s.is_variation, s.is_booster, s.game_changer, s.all_parts, s.card_faces,
        s.multiverse_ids, s.games,
        ...
        m.face_mana_value, m.edhrec_rank, m.edhrec_saltiness,
        m.is_alternative, m.is_timeshifted, m.is_rebalanced, m.is_game_changer,
        m.has_alternative_deck_limit, m.has_content_warning,
        m.printings, m.rulings, m.other_face_ids, m.card_parts, m.variations,
        m.original_supertypes, m.original_types, m.original_subtypes,
...
WITH_CANONICAL = """
,with_canonical AS (
    SELECT j.*, CASE WHEN j.uuid IS NOT NULL THEN j.uuid ELSE cm.uuid END AS canonical_uuid
    FROM joined j
    LEFT JOIN canonical_map cm
           ON j.uuid IS NULL
          AND j.set_code = cm.set_code AND j.collector_number = cm.collector_number
)"""
...
def timed(label, sql):
    ...
    con.close()
    print(f"  {label:35s}  {r:>8,} rows  {elapsed:6.1f}s")
    return elapsed

print("Phase-by-phase timing (fresh connection each, forces full eval):\n")
timed("A. joined (FULL OUTER JOIN)", BASE + JOINED + " SELECT count(*) FROM joined")
timed("B. canonical_map only",       BASE + CANONICAL_MAP + " SELECT count(*) FROM canonical_map")
timed("C. with_canonical (NL join)", BASE + JOINED + CANONICAL_MAP + WITH_CANONICAL + " SELECT count(*) FROM with_canonical")
