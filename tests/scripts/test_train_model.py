...
tests/scripts/test_check_and_retrain.py, so here ``retrain`` is mocked
to isolate train_model.main()'s own branching -- since ``retrain`` is
fully mocked, its internal ``setup_experiment()`` call (which would
otherwise point MLflow at the real project-root mlflow.db) never executes.
"""
