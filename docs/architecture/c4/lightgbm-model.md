...
**Known gap — the gate counts calendar span, not usable data.** `generate_folds()` only looks at the first and last snapshot date, so it happily returns 13 folds on the current Gold layer while `walk_forward_cv()` silently executes 2 of them: it `continue`s past any fold whose validation window contains no snapshot, or whose chosen snapshot has no exact `t+7` counterpart for `build_target()`. Nothing in the output distinguishes 13 from 2. Fixing this means counting *usable* folds — derivable from the snapshot date list alone — and it is deliberately not fixed yet, because on today's data the honest count is below the minimum of 3 and would block CV entirely. The underlying cause is Gold covering 67 of the 181 days already present in Bronze; see the Silver price-source gating issue in the dev log.
